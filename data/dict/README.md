...
- `jmdict.json` — 运行时按需载入的精简英和词典（3.93MB，30,104 常用条目 / 66,507 索引键）。
- `jlpt.json` — JLPT 难度词表（246KB，8,138 词 / 8,505 条）。来源:
  [Bluskyo/JLPT_Vocabulary](https://github.com/Bluskyo/JLPT_Vocabulary)（MIT 仓库），
  其数据出自 Jonathan Waller (tanos.co.uk) 的 JLPT 词汇表，以 **CC-BY** 发布。
  构建: `python3 scripts/build_jlpt.py`。运行时严格按词表显示 N5..N1，
  词表外显示「級外」、多读音无法对齐显示「級不明」——不猜测级别。
...
python3 scripts/build_jlpt.py             # JLPT 词表（tanos CC-BY 数据）
```
